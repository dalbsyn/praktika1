...
В словаре ERROR_CARD_NUMBERS определяется связка HPAN-карты и словаря, в которых определяется связка обстоятельства ошибки и код.

Рабочие ключи в HPAN:
- auth - ошибка из EPAY_ERROR_CODES (или успех=0) при платеже;
- charge - ошибка из EPAY_ERROR_CODES (или успех=0) при снятии средств;
- cancel - ошибка из EPAY_ERROR_CODES (или успех=0) при отмене транзакции;
- refund - ошибка из EPAY_ERROR_CODES (или успех=0) при возврате средств;
- default - ошибка из EPAY_ERROR_CODES (или успех=0) при всех вышеперечисленных операциях.

Если операция в HPAN не указана (или указана со значением 0), то она будет успешной.
"""
ERROR_CARD_NUMBERS = {
    "4000000000000001": {
        "auth": 0,
